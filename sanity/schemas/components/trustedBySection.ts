...
import { trustedBySectionInitialValue } from "@/sanity/lib/landingPageSectionInitialValues";

const trustedBySectionFields = [
  defineField({
    name: "title",
    title: "Title",
    type: "string",
...
export const trustedBySectionType = defineType({
  name: "trustedBySection",
  title: "Trusted By Section",
  type: "object",
  initialValue: trustedBySectionInitialValue,
  fields: trustedBySectionFields,
  preview: {
    select: { title: "title" },
    prepare({ title }) {
      return {
        title: title || "Trusted By",
        subtitle: "Brand logos",
      };
    },
  },
});
